...
- [x] Qwen3-ASR **audio-encoder + Qwen3-0.6B LLM decoder** (Whisper-style 128-mel HTK frontend, 8× conv2d audio tower + 18-layer transformer, 2-layer projector into a 28-layer Qwen3 LLM; greedy autoregressive decode; native Traditional Chinese + punctuation, code-switched zh/en). Jetson-Nano-gen1 optimized: cuFFT batched-mel + flash-attn disabled for sm_53. GGUF: [Luigi/qwen3-asr-0.6b-rapidspeech-gguf](https://huggingface.co/Luigi/qwen3-asr-0.6b-rapidspeech-gguf); convert with `scripts/convert_qwen3_asr_to_gguf.py` (from a local `Qwen/Qwen3-ASR-0.6B` checkpoint).
- [ ] FireRedASR2
